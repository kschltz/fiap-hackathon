    ...
    (let [mock-auth-result "mock-jwt-token"]
      (with-redefs [auth/sign (fn [_ user-data]
                                (testing "should sign the user data with the jwt secret key"
                                  (is (= "1234" (:cpf (json/read-str user-data :key-fn keyword)))))
                                mock-auth-result)]
        (xt/await-tx node (xt/submit-tx node [[::xt/put {:xt/id :user1 :cpf "1234" :senha (base/hash "password")}]]))
        (let [result (uc.login/login node {} {:cpf "1234" :senha "password"})]
